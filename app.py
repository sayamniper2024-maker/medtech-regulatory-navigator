...
RISK_COLOR  = {"Low":"#1D9E75","Medium":"#BA7517","High":"#D85A30","Critical":"#E24B4A"}
PATHWAY_KEY = {
...
FDA: Cardiovascular = Part 870 (NOT 882). Coronary stents -> Class III PMA 21 CFR 870.3945
Russia: Classes 1/2a/2b/3. RU REP required for ALL foreign manufacturers.
CDSCO: cardiac implants + joint replacements -> Class D
Health Canada: cardiac + joint -> Class IV
Japan: cardiac + joint -> Class IV, Approval, DMAH required
Australia: cardiac + joint -> Class III
"""
...
KNOWN_CORRECTIONS = [
    {
        ...
        "corrections":{
            ...
            "japan"        :{"risk_class":"IV","approval_type":"Approval",
                             "rule_applied":"PMD Act Class IV","dmah_required":"Yes","timeline_months":36},
            ...
            "russia"       :{"risk_class":"3","registration_type":"Full registration (RZN)",
                             ...
                             "clinical_investigation_required":"Yes","iso_13485_required":"Yes",
                             "local_testing_required":"Yes","site_inspection_required":"Yes",
                             "ru_rep_required":"Yes","timeline_months":24},
        },
...
def regulatory_chat(question, device_data):
    ...
    system_msg = (
        ...
        "FDA FY2026 fees: 510k=$26,067 (small biz $6,517), PMA=$579,272.\n\n"
        "Device context:\n" + "\n".join(context_lines) +
...
Extract key information from this device document. Return ONLY valid JSON.
Document: {pdf_text}
...
SUBMISSION_CHECKLISTS = {
    "fda": {
        "510(k)": ["Device description and indications for use",
        ...
        "PMA": ["Device description and principles of operation",
        ...
        "Exempt": ["Basic device description","Labeling (21 CFR Part 801)",
            "Good Manufacturing Practice (21 CFR Part 820)"],
    },
    "eu": {
        "Full Tech File": ["Device description and specification",
        ...
        "Basic UDI-DI": ["Device description","General safety and performance requirements",
            "Labeling and instructions for use","UDI assignment","Declaration of conformity"],
    },
    "cdsco": {
        "MD-14": ["Form MD-1 application","Device master file",
            "ISO 13485 certificate","Test reports from NABL/BIS lab",
            "Free Sale Certificate","Clinical performance data",
            "Manufacturer undertaking","Device description","Labeling per MDR 2017","Risk management file"],
        "MD-9": ["Form MD-1","Device master file","ISO 13485 certificate",
            "Lab test reports","Device description","Labeling","Risk management summary"],
        "MD-5": ["Form MD-1","Basic device description","Manufacturing details","Labeling"],
    },
    "health_canada": {
        "Device Licence": ["Device licence application","Device description and intended use",
            "Safety and effectiveness summary","Quality system evidence (ISO 13485)",
            "Risk analysis (ISO 14971)","Clinical evidence or literature review",
            "Labeling in English and French"],
        "MDEL only": ["MDEL application","Basic device description","Bilingual labeling"],
    },
    "japan": {
        "Approval": ["PMDA approval application","DMAH appointment","Device description",
            "Summary technical documentation (STED)","Non-clinical test data",
            "Clinical data","Manufacturing and quality control",
            "Risk analysis (ISO 14971)","JIS standard compliance","QMS ordinance compliance"],
        "Certification": ["Third-party certification application","DMAH appointment",
            "Device description","STED","Performance test data","JIS compliance","QMS compliance"],
        "Notification": ["Notification form","Basic device description","Standards conformity"],
    },
    "australia": {
        "Conformity assessment": ["ARTG application","Conformity assessment evidence",
            "Device description","Essential principles checklist","Clinical evidence",
            "Risk management (ISO 14971)","QMS evidence (ISO 13485)","Labeling in English","UDI"],
        "Self-assessment": ["ARTG application","Device description",
            "Essential principles checklist","Labeling","Declaration of conformity"],
    },
    "russia": {
        "Full registration (RZN)": ["Registration application","RU REP appointment letter",
            "Device description in Russian","Technical documentation",
            "Clinical investigation data","ISO 13485 certificate",
            "Local laboratory testing","Site inspection (Class 2b/3)",
            "Labeling in Russian","Instructions for use in Russian",
            "Risk management file (GOST R ISO 14971)"],
        "Simplified registration (RZN)": ["Registration application","RU REP appointment",
            "Device description in Russian","Local laboratory testing","Labeling in Russian"],
    },
}
...
Device: {device_data.get('device_name','Unknown')}
...
{{
  ...
  "frameworks": {{
    "framework_key": {{
      ...
      "present": ["items present"],
      "missing": ["items missing"],
      "partial": ["items incomplete"],
      "priority_gaps": ["top 3 critical missing items"],
...
def show_gap_analysis_ui(device_data, selected_fws):
    ...
    st.caption("Upload a draft submission document. AI compares it against the required checklist.")
    uploaded_doc = st.file_uploader(
        "Upload submission document (PDF)", type=["pdf"],
        key="gap_analysis_uploader",
        help="Upload your draft regulatory submission document."
    )
    if uploaded_doc is not None:
        ...
        if not success or not doc_text.strip():
            ...
            return
        st.info(f"Document loaded: {page_count} pages")
        if st.button("Run gap analysis", type="primary", key="run_gap_btn"):
            with st.spinner("Analysing against regulatory checklists..."):
                try:
    ...
    if st.session_state.get("gap_result"):
        ...
        st.markdown(
            ...
            f"padding:16px;margin:12px 0;border:0.5px solid var(--color-border-tertiary)'>"
            f"<div style='font-size:13px;color:var(--color-text-secondary)'>Overall completeness</div>"
            f"<div style='font-size:32px;font-weight:500;color:{color}'>{overall}%</div>"
            f"<div style='font-size:13px;color:var(--color-text-secondary)'>{gap.get('summary','')}</div>"
        ...
        if active_fws:
            ...
            for tab, fw in zip(gap_tabs, active_fws):
                with tab:
                    ...
                    st.markdown(
                        f"<span style='color:{clr};font-weight:600;font-size:18px'>{pct}% complete</span>  "
                        f"<span style='color:var(--color-text-secondary);font-size:13px'>{fw_gap.get('recommendation','')}</span>",
                    ...
                    with col_a:
                        ...
                        for item in present:
                            st.markdown(f"<span style='color:#1D9E75'>+ </span>{item}",unsafe_allow_html=True)
                    with col_b:
                        partial = fw_gap.get("partial",[])
                        st.markdown(f"**Partial ({len(partial)})**")
                        for item in partial:
                            st.markdown(f"<span style='color:#BA7517'>~ </span>{item}",unsafe_allow_html=True)
                    with col_c:
                        missing = fw_gap.get("missing",[])
                        st.markdown(f"**Missing ({len(missing)})**")
                        for item in missing:
                            st.markdown(f"<span style='color:#E24B4A'>x </span>{item}",unsafe_allow_html=True)
                    ...
                    if priority:
                        ...
                        for i,g in enumerate(priority,1):
                            st.markdown(
                                ...
                                f"border-radius:0 6px 6px 0;margin:4px 0;font-size:13px'>{i}. {g}</div>",
                                unsafe_allow_html=True)
...
def show_cost_estimator(data, selected_fws):
    ...
    st.caption("Approximate costs in USD. For budget planning only.")
    ...
    if not fw_labels:
        st.info("No cost data available."); return
    fig = go.Figure()
    fig.add_trace(go.Bar(name="Govt / NB fees",    x=fw_labels,y=govt,     marker_color="#378ADD"))
    fig.add_trace(go.Bar(name="Laboratory testing",x=fw_labels,y=testing,  marker_color="#1D9E75"))
    fig.add_trace(go.Bar(name="Consultant fees",   x=fw_labels,y=consult,  marker_color="#8E44AD"))
    fig.add_trace(go.Bar(name="QMS setup",         x=fw_labels,y=qms_costs,marker_color="#E67E22"))
    fig.update_layout(barmode="stack",yaxis_title="Estimated cost (USD)",
                      plot_bgcolor="white",height=400,margin=dict(t=30,b=20),
    ...
    st.warning(
        "**Cost disclaimer:** Approximate figures for planning only. "
        "FDA fees are FY2026 standard rates (510k $26,067; PMA $579,272 — "
        "small businesses may qualify for 75% reduction). "
        "EU figures represent Notified Body fees, not government fees. "
        "Always obtain formal quotes from regulatory consultants.",icon="⚠️"
    )

def show_gantt_chart(data, selected_fws):
    st.subheader("Regulatory submission timeline")
    st.caption("Estimated Gantt chart from submission start to market approval.")
    start_date = datetime.date.today()
    MILESTONES = {
        "cdsco": [("Dossier preparation",0,3),("CDSCO submission",3,1),
                  ("Query response",4,2),("Approval",6,1)],
        "fda": {
        ...
        "health_canada": [("Application prep",0,3),("HC submission",3,1),
                           ("HC review",4,5),("Licence issued",9,1)],
        "japan": {
            ...
            "Approval"     :[("STED prep",0,6),("PMDA consultation",6,3),("Application",9,1),("PMDA review",10,12),("Approval",22,1)],
        },
        "australia": {
        ...
        "russia": {
            ...
            "Full registration (RZN)"      :[("RU REP appointment",0,1),("ISO 13485",1,2),("Local testing",3,3),("Clinical data",6,3),("RZN application",9,1),("Review",10,6),("RZN certificate",16,1)],
            "EAEU registration"            :[("RU REP",0,1),("Tech dossier",1,3),("EAEU submission",4,1),("Review",5,8),("EAEU certificate",13,1)],
        },
    }
    colors = {"cdsco":"#1D9E75","fda":"#378ADD","eu":"#D85A30",
               "health_canada":"#C0392B","japan":"#8E44AD","australia":"#E67E22","russia":"#2C3E8C"}
    gantt_rows = []
    for fw in selected_fws:
        pathway  = data[fw].get(PATHWAY_KEY.get(fw,""),"")
        ms_data  = MILESTONES.get(fw,[])
        milestones = ms_data.get(pathway, list(ms_data.values())[0]) if isinstance(ms_data,dict) else ms_data
        for (task,offset,duration) in milestones:
            s = start_date + datetime.timedelta(days=int(offset*30.4))
            e = s + datetime.timedelta(days=max(1,int(duration*30.4)))
            gantt_rows.append({"Framework":FRAMEWORKS[fw]["label"],"Task":task,
                                "Start":s.strftime("%Y-%m-%d"),"Finish":e.strftime("%Y-%m-%d"),
                                "Color":colors.get(fw,"#888")})
    if not gantt_rows:
        st.info("No timeline data."); return
    df_gantt = pd.DataFrame(gantt_rows)
    fig = px.timeline(df_gantt,x_start="Start",x_end="Finish",y="Framework",
                      color="Framework",hover_data=["Task"],
                      color_discrete_map={FRAMEWORKS[fw]["label"]:colors[fw] for fw in selected_fws})
    fig.update_yaxes(autorange="reversed")
    fig.update_layout(plot_bgcolor="white",height=max(300,len(selected_fws)*80),
                      margin=dict(t=30,b=20,l=150),xaxis=dict(gridcolor="#f0f0f0"),showlegend=False)
    ...
    for fw in selected_fws:
        ...
        for (task,offset,duration) in milestones:
            ...
            mil_rows.append({"Market":FRAMEWORKS[fw]["label"],"Milestone":task,
                              "Month":f"Month {offset+1}","Est. date":mil_date.strftime("%b %Y")})
...
def generate_pdf(data, selected_fws, data2=None, selected_fws2=None,
    ...
    """Full PDF including classification, cost, gap analysis summary."""
    ...
    def write_classification(d,fws,label=""):
        ...
        if d.get("_correction_note"):
            ...
            pdf.multi_cell(PAGE_W,5,safe_pdf(f"Correction applied: {d['_correction_note']}"))
    ...
    def write_cost_section(d, fws):
        dv(); sh("Regulatory Cost Estimates",(55,100,170))
        pdf.set_font("Helvetica","B",9)
        cw5=PAGE_W//5
        for h in ["Market","Govt/NB ($)","Testing ($)","Consultant ($)","Total ($)"]:
            pdf.cell(cw5,6,h,border=1,ln=0)
        pdf.ln(); pdf.set_font("Helvetica","",9)
        for fw in fws:
            pathway  = d[fw].get(PATHWAY_KEY.get(fw,""),"")
            fw_costs = REGULATORY_COSTS.get(fw,{})
            costs    = fw_costs.get(pathway, list(fw_costs.values())[0] if fw_costs else {})
            if not costs: continue
            total = sum(costs.values())
            pdf.cell(cw5,6,safe_pdf(FRAMEWORKS[fw]["label"][:15]),border=1,ln=0)
            pdf.cell(cw5,6,safe_pdf(f"${costs.get('govt_fees',0):,.0f}"),border=1,ln=0)
            pdf.cell(cw5,6,safe_pdf(f"${costs.get('testing',0):,.0f}"),border=1,ln=0)
            pdf.cell(cw5,6,safe_pdf(f"${costs.get('consultant',0):,.0f}"),border=1,ln=0)
            pdf.cell(cw5,6,safe_pdf(f"${total:,.0f}"),border=1,ln=True)
        pdf.set_font("Helvetica","I",8); pdf.set_text_color(120,120,120)
        pdf.multi_cell(PAGE_W,5,safe_pdf(
            "Estimates for planning only. FDA FY2026 standard fees. "
            "Small business 510k fee: $6,517. Consult professionals for formal quotes."))
        pdf.set_text_color(0,0,0); pdf.ln(3)

    def write_gap_section(gap_result, fws):
        if not gap_result: return
        dv(); sh("Submission Document Gap Analysis",(180,80,0))
        overall = gap_result.get("overall_completeness",0)
        pdf.set_font("Helvetica","B",11)
        pdf.set_text_color(30,158,117 if overall>=70 else 0)
        pdf.cell(PAGE_W,7,safe_pdf(f"Overall completeness: {overall}%"),ln=True)
        pdf.set_text_color(0,0,0)
        pdf.set_font("Helvetica","",9)
        pdf.multi_cell(PAGE_W,6,safe_pdf(gap_result.get("summary","")))
        pdf.ln(3)
        fw_results = gap_result.get("frameworks",{})
        for fw in fws:
            if fw not in fw_results: continue
            fw_gap = fw_results[fw]
            pct    = fw_gap.get("completeness_percent",0)
            pdf.set_font("Helvetica","B",10); pdf.set_text_color(50,50,50)
            pdf.cell(PAGE_W,7,safe_pdf(f"  {FRAMEWORKS[fw]['label']} - {pct}% complete"),ln=True)
            pdf.set_text_color(0,0,0); pdf.set_font("Helvetica","",8)
            for item in fw_gap.get("present",[])[:5]:
                pdf.cell(PAGE_W,5,safe_pdf(f"  + {item}"),ln=True)
            for item in fw_gap.get("missing",[])[:5]:
                pdf.cell(PAGE_W,5,safe_pdf(f"  x MISSING: {item}"),ln=True)
            priority = fw_gap.get("priority_gaps",[])
            if priority:
                pdf.set_font("Helvetica","B",8)
                pdf.cell(PAGE_W,5,safe_pdf("  Priority gaps:"),ln=True)
                pdf.set_font("Helvetica","",8)
                for i,g in enumerate(priority,1):
                    pdf.cell(PAGE_W,5,safe_pdf(f"  {i}. {g}"),ln=True)
            pdf.ln(2)
    ...
    pdf.cell(PAGE_W,6,"Complete regulatory analysis - 7 global frameworks",ln=True,align="C")
    ...
    if data2 and selected_fws2:
        ...
        pdf.set_text_color(0,0,0); pdf.ln(4)
        write_classification(data, selected_fws, "DEVICE 1")
        pdf.add_page()
        write_classification(data2, selected_fws2, "DEVICE 2")
    else:
        write_classification(data, selected_fws)

    write_cost_section(data, selected_fws)
    write_gap_section(gap_result, selected_fws)
...
def show_device_results(data, selected_fws, prefix=""):
    ...
    disclaimer  = data.get("disclaimer","").strip() or (
        "AI-generated. Verify with a qualified regulatory affairs professional.")
    st.info(f"**{data['device_name']}** -- {data['intended_use']}")
    st.markdown(f"AI confidence: :{conf_colors.get(conf,'orange')}[**{conf}**]  |  _{disclaimer}_")
    ...
    fig_bar = go.Figure(go.Bar(x=b_labels,y=b_timelines,marker_color=b_colors,
    ...
    fig_bar.add_hline(y=avg,line_dash="dot",line_color="#888",
                      annotation_text=f"Avg {avg:.0f} mo",annotation_position="top right")
    fig_bar.update_layout(yaxis_title="Months to approval",plot_bgcolor="white",
                          height=380,margin=dict(t=30,b=20),yaxis=dict(gridcolor="#f0f0f0"))
        ...
        with tab:
            ...
            with dc2:
                ...
                st.markdown(
                    ...
                    f"Class {rc} -- {lvl} Risk</span>",unsafe_allow_html=True)
    st.divider()
    ...
    for fw in selected_fws:
        summary[FRAMEWORKS[fw]["label"]] = [
            ...
            data[fw].get("qms_required",data[fw].get("iso_13485_required",
...
with st.sidebar:
    ...
    if input_mode == "Type any device name":
        with st.expander("Upload device datasheet PDF (auto-fill)", expanded=False):
            uploaded_pdf = st.file_uploader(
                "Upload PDF", type=["pdf"], key="sidebar_pdf_uploader",
                label_visibility="collapsed",
                help="Upload any device spec sheet. AI extracts name and description."
            )
            if uploaded_pdf is not None:
                ...
                if success and pdf_text.strip():
                    ...
                    st.warning("No text found. Use a text-based PDF.")

        device_name = st.text_input("Device name",
                                    value=st.session_state.get("pdf_device_name",""),
                                    placeholder="e.g. Coronary Stent or upload PDF above")
        device_desc = st.text_area("Description (optional)",
                                   value=st.session_state.get("pdf_device_desc",""),height=80)
    ...
    if st.session_state.search_history:
        ...
        st.download_button("Export CSV",data=csv_bytes,file_name="regulatory_history.csv",
...
if "reload_data" in st.session_state:
    ...
    st.session_state["last_data"]=data; st.session_state["last_data2"]=None
    st.session_state["last_fws"]=selected_fws; st.session_state["last_compare"]=False
    st.session_state["current_device"]=data; st.session_state["chat_history"]=[]
    ...
    data=st.session_state.get("last_data"); data2=st.session_state.get("last_data2")
...
if analyse_show and data:
    ...
    # 4. Chatbot (just before disclaimer)
    ...
    if st.session_state.chat_history:
        ...
        if st.button("Clear conversation",key="clear_chat"):
            st.session_state["chat_history"]=[]; st.session_state["chat_input_counter"]+=1
            st.rerun()
    else:
    ...
    # 5. PDF download (after all analysis, before disclaimer)
    st.subheader("Download full report")
    st.caption("PDF includes classification, timelines, cost estimates, and gap analysis summary.")
    ...
    with ex1:
        if compare_mode and data2:
            ...
            pdf_bytes = generate_pdf(data, selected_fws,
                                     gap_result=st.session_state.get("gap_result"))
            st.download_button("Download full PDF", data=pdf_bytes,
                file_name=f"{data['device_name'].replace(' ','_')}_full_report.pdf",
                mime="application/pdf", type="primary", use_container_width=True)
    with ex2:
        st.caption(
            f"Complete analysis: {len(selected_fws)} markets | "
            "Classification + timelines + cost estimates + gap analysis"
        )
